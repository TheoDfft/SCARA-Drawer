...
class MarkerPoseFusion:
    ...
    def _fuse_and_publish(self):
        ...
        if self.latest_pose_1 is not None and self.latest_pose_2 is not None:
            ...
            fused_pose.pose.position.x = (self.latest_pose_1.pose.position.x + self.latest_pose_2.pose.position.x) / 2
            fused_pose.pose.position.y = (self.latest_pose_1.pose.position.y + self.latest_pose_2.pose.position.y) / 2
            fused_pose.pose.position.z = (self.latest_pose_1.pose.position.z + self.latest_pose_2.pose.position.z) / 2

            # -----------------------------

            self._fused_pose_pub.publish(fused_pose)
